...
export default function SignUp() {
    ...
    const handleGoogleSignUp = async () => {
        ...
        try {
            ...
            // Create user doc if not exists (merge: true handles idempotency)
            await setDoc(doc(db, "users", user.uid), {
            ...
            <div className="w-full max-w-lg glass-morphism rounded-[3rem] shadow-2xl p-12 sm:p-20 space-y-12 animate-in fade-in zoom-in duration-700 border border-ocean/5 relative z-10 premium-card">
                ...
                <form onSubmit={handleEmailSignUp} className="space-y-8">
                    <div className="space-y-3">
                        <label className="block text-[10px] font-black text-ocean/30 uppercase tracking-[0.4em] ml-2">Como te chamamos?</label>
                        <input
                            type="text"
                            value={name}
                    ...
                    >
                        {loading ? "Criando..." : "Gerar Passe VIP ⚡"}
                    </button>
                </form>
                ...
                <p className="text-center text-[10px] font-black text-ocean/40 uppercase tracking-widest">
                    Já tem acesso?{" "}
                    <Link to="/login" className="text-coral hover:underline ml-2">
                        Entrar
                    </Link>
                </p>
